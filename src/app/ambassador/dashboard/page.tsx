...
type Reward = {
  ...
  earnedAt: string | null; paidAt: string | null;
...
const rewardStatus: Record<RewardStatus, string> = { EXPECTED: "متوقعة", EARNED: "مستحقة", PAID: "مدفوعة", CANCELLED: "ملغاة" };
const rewardStatusClass: Record<RewardStatus, string> = {
  ...
  EARNED: "bg-blue-100 text-blue-800 dark:bg-blue-950/50 dark:text-blue-200",
  PAID: "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-200",
...
export default function AmbassadorDashboardPage() {
  ...
  async function addReferral(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(dashboardErrorMessage(payload.error, "تعذر إضافة الإحالة"));
      ...
      formElement.reset();
      setNotice("تم تسجيل الإحالة المباشرة وربطها بحسابك. الإدارة ستتابع حالتها من هنا.");
    } catch (cause) {
      ...
      <main className="min-h-screen lg:mr-[310px]">
        ...
        <div className="mx-auto max-w-7xl space-y-7 p-4 sm:p-7 lg:p-10">
          ...
          {activeSection === "tools" && <section className="space-y-7">
            ...
            <form onSubmit={addReferral} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8 dark:border-slate-700 dark:bg-slate-900"><div className="flex items-center gap-3"><span className="rounded-2xl bg-[#f3ead7] p-3 text-[#9f7d3d] dark:bg-[#bd9850]/15"><PlusCircle size={23} /></span><div><p className="text-sm font-black text-[#9f7d3d]">إضافة من تواصل مباشر</p><h3 className="text-2xl font-black">إحالة مباشرة</h3></div></div><div className="mt-6 grid gap-4 md:grid-cols-2"><label className="grid gap-2 text-sm font-bold">اسم العميل<input required name="name" maxLength={120} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold">الهاتف أو وسيلة التواصل<input required name="contactMethod" maxLength={160} placeholder="رقم واتساب، حساب تواصل، اتصال..." className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold">البريد الإلكتروني — اختياري<input name="email" type="email" maxLength={254} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold">الشركة — اختياري<input name="company" maxLength={160} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold md:col-span-2">ماذا يحتاج العميل؟<textarea required name="needs" maxLength={2000} rows={4} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold md:col-span-2">ملاحظات — اختياري<textarea name="notes" maxLength={2000} rows={3} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label></div><button disabled={addingReferral} className="mt-5 rounded-xl bg-slate-950 px-6 py-3 font-black text-white disabled:opacity-60 dark:bg-[#bd9850] dark:text-slate-950">{addingReferral ? "جارٍ التسجيل..." : "تسجيل الإحالة المباشرة"}</button></form>
          ...
          {activeSection === "referrals" && <section className="space-y-7"><div><p className="text-sm font-black text-[#9f7d3d]">مسارك الخاص</p><h2 className="mt-1 text-3xl font-black">إحالاتي</h2><p className="mt-2 text-slate-600 dark:text-slate-300">ترى إحالاتك فقط؛ قرارات الحالة والمكافأة تبقى بيد الإدارة.</p></div>
            <form onSubmit={addReferral} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8 dark:border-slate-700 dark:bg-slate-900"><div className="flex items-center gap-3"><span className="rounded-2xl bg-[#f3ead7] p-3 text-[#9f7d3d] dark:bg-[#bd9850]/15"><PlusCircle size={23} /></span><div><h3 className="text-xl font-black">إحالة مباشرة</h3><p className="text-sm text-slate-500">سجّل عميلًا محتملًا تعرفه مباشرة وسيُربط بحسابك تلقائيًا.</p></div></div><div className="mt-6 grid gap-4 md:grid-cols-2"><label className="grid gap-2 text-sm font-bold">اسم العميل<input required name="name" maxLength={120} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold">الهاتف أو وسيلة التواصل<input required name="contactMethod" maxLength={160} placeholder="رقم واتساب، حساب تواصل، اتصال..." className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold">البريد الإلكتروني — اختياري<input name="email" type="email" maxLength={254} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold">الشركة — اختياري<input name="company" maxLength={160} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold">رقم الهاتف — اختياري<input name="phone" maxLength={40} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold md:col-span-2">ماذا يحتاج العميل؟<textarea required name="needs" maxLength={2000} rows={4} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid gap-2 text-sm font-bold md:col-span-2">ملاحظات — اختياري<textarea name="notes" maxLength={2000} rows={3} className="rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-[#bd9850] dark:border-slate-700" /></label></div><button disabled={addingReferral} className="mt-5 rounded-xl bg-slate-950 px-6 py-3 font-black text-white disabled:opacity-60 dark:bg-[#bd9850] dark:text-slate-950">{addingReferral ? "جارٍ التسجيل..." : "تسجيل الإحالة المباشرة"}</button></form>
