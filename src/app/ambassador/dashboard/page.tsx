...
export default function AmbassadorDashboardPage() {
  ...
  useEffect(() => {
    queueMicrotask(() => {
      setDarkMode(readLocalStorage("cyberweel-ambassador-theme") === "dark");
  ...
  function openTool(id: AmbassadorToolId) {
  ...
  function toggleDarkMode() {
    setDarkMode((current) => {
      const next = !current;
      writeLocalStorage("cyberweel-ambassador-theme", next ? "dark" : "light");
      return next;
    });
  }
  ...
  return (
    <div dir="rtl" className={darkMode ? "dark min-h-screen bg-slate-950 text-white" : "min-h-screen bg-[#f5f1e8] text-slate-950"}>
      ...
      <aside id="ambassador-dashboard-menu" inert={!desktopSidebar && !menuOpen} aria-hidden={!desktopSidebar && !menuOpen ? true : undefined} aria-label="قائمة لوحة السفير" className={`fixed inset-y-0 right-0 z-50 flex w-[310px] max-w-[calc(100vw-1rem)] flex-col overflow-y-auto overscroll-contain bg-[#101827] p-4 text-white shadow-2xl transition-transform sm:p-6 lg:translate-x-0 ${menuOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex items-start justify-between gap-3"><DashboardWordmark /><button type="button" aria-label="إغلاق القائمة" onClick={() => setMenuOpen(false)} className="rounded-xl p-2 text-white/70 hover:bg-white/10 lg:hidden"><X size={22} /></button></div>
        ...
        <div className="mt-auto space-y-3"><Link href="/" className="flex items-center justify-center gap-2 rounded-2xl bg-[#bd9850] px-4 py-3 font-black text-slate-950"><ArrowLeft size={18} />العودة إلى الموقع</Link><button type="button" onClick={logout} disabled={loggingOut} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-white/15 px-4 py-3 font-black text-white/80 transition hover:bg-white/10 disabled:opacity-60 sm:hidden"><LogOut size={18} />{data?.isAdminPreview ? "العودة للإدارة" : loggingOut ? "جارٍ الخروج" : "تسجيل الخروج"}</button><div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm leading-7 text-white/65">تحتاج إلى مساعدة؟<br /><Link href="/contact" className="font-black text-[#d5b873]">تواصل معنا</Link></div></div>
      </aside>
      ...
      <main className="min-h-screen lg:mr-[310px]">
        <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-[#f5f1e8]/90 px-4 py-4 backdrop-blur sm:px-7 lg:px-10 dark:border-slate-800 dark:bg-slate-950/90">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 sm:flex-nowrap"><div className="flex min-w-0 flex-1 items-center gap-3"><button type="button" aria-label="فتح القائمة" aria-expanded={menuOpen} aria-controls="ambassador-dashboard-menu" onClick={() => setMenuOpen(true)} className="min-h-11 min-w-11 shrink-0 rounded-xl border border-slate-200 bg-white p-2.5 shadow-sm lg:hidden dark:border-slate-700 dark:bg-slate-900"><Menu size={21} /></button><div className="min-w-0"><p className="text-xs font-black tracking-[0.14em] text-[#9f7d3d]">{data.isAdminPreview ? "معاينة الإدارة · للقراءة فقط" : "لوحة سفير CyberWeel"}</p><h1 className="mt-1 break-words text-lg font-black sm:text-2xl">مرحبًا، {data.ambassador.name}</h1></div></div><div className="flex w-full shrink-0 items-center justify-end gap-2 sm:w-auto"><button aria-label="تبديل المظهر" onClick={toggleDarkMode} className="min-h-11 min-w-11 rounded-xl border border-slate-200 bg-white p-2.5 shadow-sm dark:border-slate-700 dark:bg-slate-900">{darkMode ? <Sun size={20} /> : <Moon size={20} />}</button><button type="button" onClick={logout} disabled={loggingOut} className="hidden items-center gap-2 rounded-xl bg-rose-600 px-4 py-2.5 font-black text-white hover:bg-rose-700 disabled:opacity-60 sm:flex"><LogOut size={18} />{data.isAdminPreview ? "العودة للإدارة" : loggingOut ? "جارٍ الخروج" : "تسجيل الخروج"}</button></div></div>
        </header>
        ...
        <div className="mx-auto max-w-7xl space-y-7 p-4 sm:p-7 lg:p-10">
          ...
          {activeSection === "tools" && <section className="min-w-0 space-y-7">
            ...
            <AmbassadorToolsWorkspace data={data} copied={copied} onCopyReferralLink={copyReferralLink} onOpenTool={openTool} onNavigate={navigate} />
            ...
            <form id="ambassador-assistant" tabIndex={-1} onSubmit={askAssistant} className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8 dark:border-slate-700 dark:bg-slate-900">
              ...
              <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-500">احكِ لي عن العميل أو الرسالة اللي بدك تكتبها، وبساعدك بصياغة بسيطة وقريبة من الناس وبنفس لغتك. رابطك بينضاف تلقائيًا لرسالة واتساب؛ والأسعار والمواعيد بتبقى مع الإدارة.</p>
              <div className="mt-6 grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
                <label className="grid min-w-0 content-start gap-2 text-sm font-bold">نوع المساعدة<select value={assistantMode} onChange={(event) => setAssistantMode(event.target.value as AssistantMode)} className="min-h-12 w-full min-w-0 rounded-xl border border-slate-200 bg-transparent px-3 py-3 text-base outline-none focus:border-[#bd9850] dark:border-slate-700">{assistantModes.map((mode) => <option key={mode.value} value={mode.value}>{mode.label}</option>)}</select></label>
                <label className="grid min-w-0 gap-2 text-sm font-bold">حالة العميل<textarea required minLength={10} maxLength={2500} rows={5} value={assistantSituation} onChange={(event) => setAssistantSituation(event.target.value)} placeholder="مثال: بدي أشارك رابطي بمجموعة واتساب، بكلام بسيط يفهمه الكل." className="w-full min-w-0 rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-base outline-none focus:border-[#bd9850] dark:border-slate-700" /></label>
              </div>
              <button disabled={data.isAdminPreview || askingAssistant || assistantSituation.trim().length < 10} className="mt-5 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#B89A5A] px-6 py-3 font-black text-[#111827] disabled:opacity-60 sm:w-auto"><Send size={18} />{askingAssistant ? "جارٍ تجهيز الرد..." : "اطلب المساعدة"}</button>
              <div aria-live="polite" aria-busy={askingAssistant}>
                {assistantAnswer && <div className="mt-6 min-w-0 rounded-2xl border border-[#D8D2C4] bg-[#F7F3EB] p-4 text-slate-800 sm:p-5 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"><p dir="auto" data-assistant-answer data-no-dashboard-translate className="select-text whitespace-pre-wrap leading-8 [overflow-wrap:anywhere]">{assistantAnswer}</p><button type="button" onClick={() => copyValue(assistantAnswer, "assistant-answer")} className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 font-black text-white sm:w-auto dark:bg-[#bd9850] dark:text-slate-950">{copiedContent === "assistant-answer" ? <Check size={17} /> : <Copy size={17} />}{copiedContent === "assistant-answer" ? "تم النسخ" : "نسخ الرد"}</button></div>}
              </div>
            </form>

            <div id="ambassador-ready-content" tabIndex={-1}><div className="mb-4 flex items-center gap-3"><span className="rounded-2xl bg-[#f3ead7] p-3 text-[#9f7d3d] dark:bg-[#bd9850]/15"><BookOpenText size={23} /></span><div><p className="text-sm font-black text-[#9f7d3d]">جاهز للاستخدام</p><h3 className="text-2xl font-black">محتوى جاهز للمشاركة</h3></div></div><div className="grid gap-4 lg:grid-cols-2">{readyContent.map((item) => { const itemText = item.text[lang]; const itemTitle = item.title[lang]; return <article key={item.id} className="flex flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900"><h4 className="text-lg font-black">{itemTitle}</h4><p className="mt-3 flex-1 whitespace-pre-wrap text-sm leading-7 text-slate-600 dark:text-slate-300">{itemText}</p><div className="mt-5 flex gap-2"><button type="button" onClick={() => copyValue(itemText, item.id)} className="flex min-h-11 items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-black text-white dark:bg-[#bd9850] dark:text-slate-950">{copiedContent === item.id ? <Check size={17} /> : <Copy size={17} />}{copiedContent === item.id ? "تم النسخ" : "نسخ"}</button><button type="button" onClick={() => shareValue({ title: itemTitle, text: itemText, url: data.ambassador.referralUrl, fallbackKey: `share-${item.id}` })} className="flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-black dark:border-slate-700"><Share2 size={17} />مشاركة</button></div></article>; })}</div></div>
          </section>}
          ...
          {activeSection === "rewards" && <section className="space-y-7"><div><p className="text-sm font-black text-[#9f7d3d]">وضوح من الإحالة حتى الدفع</p><h2 className="mt-1 text-3xl font-black">مكافآتي</h2><p className="mt-2 text-slate-600 dark:text-slate-300">مكافأتك الكاملة تظهر كرصيد متوقع من المشروع، وتتحول أجزاؤها إلى مدفوعة مع اكتمال شروط المراحل وتسجيل الدفع.</p></div>
            ...
            <div className="rounded-3xl bg-[#101827] p-6 text-white shadow-xl sm:p-8"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-center"><div><p className="text-sm font-black text-[#d5b873]">مستواك هذا الشهر</p><h3 className="mt-2 text-3xl font-black">{data.stats.monthlyLevel.name}</h3><p className="mt-2 text-white/65">الإحالات الناجحة ماليًا: {data.stats.monthlyLevel.successfulReferrals} · النسبة الحالية: {data.stats.monthlyLevel.rate}%</p></div><div className="w-full min-w-0 md:w-[260px]"><div className="mb-2 flex justify-between text-sm"><span>{data.stats.monthlyLevel.successfulReferrals} من {data.stats.monthlyLevel.nextTarget || data.stats.monthlyLevel.successfulReferrals}</span><span>{data.stats.monthlyLevel.nextRate ? `${data.stats.monthlyLevel.nextRate}%` : "أعلى مستوى"}</span></div><div className="h-3 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-[#bd9850]" style={{ width: `${data.stats.monthlyLevel.nextTarget ? Math.min(100, data.stats.monthlyLevel.successfulReferrals / data.stats.monthlyLevel.nextTarget * 100) : 100}%` }} /></div><p className="mt-3 text-sm text-white/65">{data.stats.monthlyLevel.nextRate ? `بقيت ${data.stats.monthlyLevel.remaining} إحالات ناجحة ماليًا للوصول إلى نسبة ${data.stats.monthlyLevel.nextRate}% هذا الشهر.` : "أنت في أعلى مستوى لهذا الشهر."}</p></div></div></div>
          ...
          {activeSection === "profile" && <section className="mx-auto max-w-3xl space-y-6"><div><p className="text-sm font-black text-[#9f7d3d]">بياناتك المالية والتواصلية</p><h2 className="mt-1 text-3xl font-black">الملف الشخصي</h2><p className="mt-2 text-slate-600 dark:text-slate-300">حدّث بيانات التواصل وطريقة استلام المكافآت من مكان واحد.</p></div><form onSubmit={saveProfile} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8 dark:border-slate-700 dark:bg-slate-900"><div className="grid gap-4 sm:grid-cols-2"><label className="grid min-w-0 gap-2 text-sm font-bold">رقم التواصل<input required name="phone" maxLength={40} defaultValue={data.ambassador.phone || ""} className="w-full min-w-0 rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-base outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid min-w-0 gap-2 text-sm font-bold">البلد<input required name="country" maxLength={100} defaultValue={data.ambassador.country || ""} className="w-full min-w-0 rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-base outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid min-w-0 gap-2 text-sm font-bold">طريقة التواصل المفضلة<input required name="contactMethod" maxLength={100} defaultValue={data.ambassador.contactMethod || ""} className="w-full min-w-0 rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-base outline-none focus:border-[#bd9850] dark:border-slate-700" /></label><label className="grid min-w-0 gap-2 text-sm font-bold">طريقة استلام المكافأة<select required name="payoutMethod" defaultValue={data.ambassador.payoutMethod || ""} className="w-full min-w-0 rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-base outline-none focus:border-[#bd9850] dark:border-slate-700"><option value="" disabled>اختر طريقة الاستلام</option><option value="شام كاش">شام كاش</option><option value="حوالة مالية">حوالة مالية</option><option value="تحويل بنكي">تحويل بنكي</option><option value="أخرى">أخرى</option></select></label><label className="grid min-w-0 gap-2 text-sm font-bold sm:col-span-2">بيانات الاستلام<textarea required name="payoutDetails" maxLength={2000} rows={4} defaultValue={data.ambassador.payoutDetails || ""} className="w-full min-w-0 rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-base outline-none focus:border-[#bd9850] dark:border-slate-700" /></label></div><button disabled={savingProfile} className="mt-5 rounded-xl bg-slate-950 px-6 py-3 font-black text-white disabled:opacity-60 dark:bg-[#bd9850] dark:text-slate-950">{savingProfile ? "جارٍ الحفظ..." : "حفظ البيانات"}</button></form><div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900"><dl className="grid gap-4 sm:grid-cols-2"><div><dt className="text-sm text-slate-500">البريد الإلكتروني</dt><dd className="mt-1 break-all font-black">{data.ambassador.email}</dd></div><div><dt className="text-sm text-slate-500">سفير منذ</dt><dd className="mt-1 font-black"><DateText value={data.ambassador.joinedAt} /></dd></div></dl><Link href="/partner/forgot-password" className="mt-5 inline-flex items-center gap-2 font-black text-[#9f7d3d]">تغيير كلمة المرور <ArrowLeft size={17} /></Link></div></section>}
        </div>
      </main>
    </div>
  );
