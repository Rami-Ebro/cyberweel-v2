...
type DashboardData = {
  ambassador: { name: string; code: string; referralUrl: string };
...
export default function AmbassadorActionCenterPage() {
  ...
  useEffect(() => {
    ...
    fetch(endpoint, { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(payload?.error || "تعذر تحميل مركز العمل");
  ...
  const remainingRewards = data?.stats.rewardsByCurrency.reduce(
    (sum, item) => sum + Number(item.expected || 0) + Number(item.earned || 0),
    0,
  ) ?? 0;
  ...
  if (error) {
    return <main dir="rtl" className="grid min-h-screen place-items-center bg-[#F7F3EB] p-6"><div className="max-w-lg rounded-3xl bg-white p-8 text-center shadow-sm"><h1 className="text-2xl font-black">تعذر تحميل مركز العمل</h1><p className="mt-3 text-slate-600">{error}</p><Link href="/ambassador/dashboard" className="mt-6 inline-flex rounded-xl bg-slate-950 px-5 py-3 font-black text-white">العودة إلى اللوحة</Link></div></main>;
  }
    ...
    <main dir="rtl" className="min-h-screen bg-[#F7F3EB] px-4 py-6 text-slate-950 sm:px-7 lg:px-10">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="flex flex-col justify-between gap-4 rounded-3xl bg-[#111827] p-6 text-white shadow-xl sm:flex-row sm:items-center sm:p-8">
          ...
          <Link href="/ambassador/dashboard" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/15 px-5 py-3 font-black text-white"><ArrowLeft size={18} />اللوحة الحالية</Link>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><span className="rounded-2xl bg-[#f3ead7] p-3 text-[#9f7d3d]"><BadgeDollarSign /></span><strong className="text-3xl font-black">{remainingRewards.toFixed(2)}</strong></div><h2 className="mt-5 text-lg font-black">مكافآت غير مدفوعة</h2><p className="mt-2 text-sm leading-6 text-slate-500">إجمالي المتوقع والمستحق عبر العملات؛ التفاصيل في لوحة المكافآت.</p></article>
        </section>
            ...
            <div className="mt-5 space-y-3">
              {followUps.slice(0, 5).map((referral) => (
                <article key={referral.id} className="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 p-5 sm:flex-row sm:items-center">
                  ...
                  <Link href="/ambassador/dashboard" className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-black text-white"><MessageCircle size={17} />متابعة الإحالة</Link>
                </article>
              ))}
        ...
        <section className="grid gap-4 sm:grid-cols-3">
          <Link href="/ambassador/dashboard" className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5"><UserPlus className="text-[#9f7d3d]" /><h3 className="mt-4 text-lg font-black">أضف إحالة</h3><p className="mt-2 text-sm text-slate-500">سجّل عميلًا محتملًا مباشرة.</p></Link>
          <Link href="/ambassador/dashboard" className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5"><Sparkles className="text-[#9f7d3d]" /><h3 className="mt-4 text-lg font-black">جهّز رسالة</h3><p className="mt-2 text-sm text-slate-500">استخدم مساعد Gemini الحالي بصلاحياته الآمنة.</p></Link>
          <Link href="/ambassador/dashboard" className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5"><UsersRound className="text-[#9f7d3d]" /><h3 className="mt-4 text-lg font-black">راجع كل الإحالات</h3><p className="mt-2 text-sm text-slate-500">شاهد الحالة والنتائج من المصدر الحالي.</p></Link>
        </section>
      </div>
    </main>
  );
