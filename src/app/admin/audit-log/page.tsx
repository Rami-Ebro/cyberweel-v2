    ...
    <main dir="rtl" className="min-h-screen bg-[#F7F3EB] p-4 text-[#111827] sm:p-8">
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-sm font-bold text-[#9A7D43]">لوحة المالك</p>
          ...
          {loading ? <div className="rounded-2xl bg-white p-10 text-center">جارٍ تحميل السجل...</div> : data?.logs.length ? data.logs.map((log) => (
            <article key={log.id} className="rounded-2xl border border-[#D8D2C4] bg-white p-5 shadow-sm">
              <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
                ...
                <div className="text-sm lg:text-left"><p className="font-black">{log.actorName || "حساب إدارة"}</p><p dir="ltr" className="mt-1 text-slate-500">{log.actorEmail}</p><time dateTime={log.createdAt} className="mt-2 block text-xs font-bold text-[#9A7D43]">{new Date(log.createdAt).toLocaleString("ar")}</time></div>
              </div>
