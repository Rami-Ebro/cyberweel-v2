...
export default function AdminRewardsPage() {
  ...
  return <AdminShell active="rewards" title="مكافآت السفراء" description="مراحل مالية واضحة، نسب محفوظة، واستحقاق لا يسبق الإنجاز والاعتماد والدفع.">
    ...
    {loading ? <p className="mt-7 flex items-center justify-center gap-2 rounded-2xl bg-white p-12"><RefreshCw className="animate-spin" /> جارٍ التحميل...</p> : <>
      ...
      <details className="mt-6 rounded-2xl border border-[#D8D2C4] bg-white p-5"><summary className="flex cursor-pointer list-none items-center justify-between font-black"><span>إعدادات مستويات المكافآت</span><BadgeDollarSign /></summary><div className="mt-5 grid gap-3">{levels.map((level) => <form key={level.id} onSubmit={(event) => saveLevel(event, level.id)} className="grid gap-3 rounded-xl bg-[#F7F3EB] p-4 md:grid-cols-5"><input name="name" defaultValue={level.name} required className="field" /><input name="minimum" type="number" min="1" defaultValue={level.minSuccessfulReferrals} required className="field" /><input name="rate" type="number" min="0.01" max="100" step="0.01" defaultValue={level.rate} required className="field" /><label className="flex items-center gap-2"><input name="active" type="checkbox" defaultChecked={level.isActive} /> فعال</label><button className="rounded-xl bg-[#B89A5A] px-4 py-2 font-black">حفظ</button></form>)}<form onSubmit={(event) => saveLevel(event)} className="grid gap-3 rounded-xl border border-dashed border-[#B89A5A] p-4 md:grid-cols-5"><input name="name" required placeholder="اسم مستوى جديد" className="field" /><input name="minimum" required type="number" min="1" placeholder="عدد الإحالات" className="field" /><input name="rate" required type="number" min="0.01" max="100" step="0.01" placeholder="النسبة %" className="field" /><label className="flex items-center gap-2"><input name="active" type="checkbox" defaultChecked /> فعال</label><button className="rounded-xl bg-[#111827] px-4 py-2 font-black text-white">إضافة مستوى</button></form></div></details>
    </>}
  </AdminShell>;
}
