...
export default function ReferralAdmin() {
  ...
  return (
    <AdminShell active="referrals" title="إدارة الإحالات" description="مسار التواصل وقرار الإدارة واستحقاق العمولة في شاشة واحدة واضحة.">
      {message && <div role="status" className="fixed left-6 top-6 z-[70] grid min-h-24 w-36 place-items-center rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-center font-black text-emerald-800 shadow-xl animate-pulse">{message}</div>}
...
function ReferralEditor({
  ...
  async function convertToClient(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setConversionMessage(data.reusedExistingClient ? "تم الربط بالعميل الموجود" : "تم التحويل بنجاح");
      ...
      <article className="rounded-2xl border border-[#E2DACB] bg-white p-5 shadow-sm">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div className="grid flex-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <SummaryFact label="حالة الإحالة" value={<span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${statusColors[referral.status]}`}>{referralLabels[referral.status]}</span>} />
          ...
          <button type="button" aria-expanded="false" onClick={onExpand} className="flex shrink-0 items-center justify-center gap-2 rounded-xl border border-[#B89A5A] bg-[#FCFAF6] px-5 py-3 font-black text-[#9A7D43] transition hover:bg-[#F7F3EB]">عرض التفاصيل<ChevronDown className="h-4 w-4" /></button>
        </div>
      </article>
    );
  }
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl border border-[#E2DACB] bg-white shadow-sm">
      <header className="order-0 flex flex-wrap items-start justify-between gap-4 border-b border-[#EEE7DA] bg-[#FCFAF6] p-5">
        <div>
          <div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-black">{referral.name || "دون اسم"}</h2><span className={`rounded-full px-3 py-1 text-xs font-bold ${statusColors[draft.status]}`}>{referralLabels[draft.status]}</span></div>
          <p className="mt-2 text-sm text-slate-600">{referral.email || "—"}{referral.phone ? ` · ${referral.phone}` : ""}</p>
        </div>
        <div className="flex items-start gap-3">
          <div className="grid gap-1 text-xs text-slate-500 sm:text-left"><span>{sourceLabels[referral.source || ""] || "إحالة مباشرة"} · {referral.contactMethod || "وسيلة التواصل غير محددة"}</span><span>{owner ? `عن طريق ${owner.name || owner.email}` : "دون مسوّق مرتبط"} · <DateText value={referral.createdAt} /></span></div>
          <button type="button" aria-expanded="true" onClick={onCollapse} className="flex shrink-0 items-center gap-1 rounded-lg border border-[#D8D2C4] bg-white px-3 py-2 text-xs font-black text-slate-600 hover:bg-[#F7F3EB]">طي التفاصيل<ChevronDown className="h-4 w-4 rotate-180" /></button>
        </div>
      </header>

      <section className="order-1 mx-5 mt-5 rounded-2xl border border-[#D8D2C4] bg-[#FCFAF6] p-4">
        {referral.convertedClient ? (
          <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black">بيانات العميل</h3><p className="mt-1 text-sm text-slate-600">تم التحويل والربط مع {referral.convertedClient.name || referral.convertedClient.email}</p></div><span className="rounded-xl bg-emerald-100 px-4 py-2 font-black text-emerald-800">تم التحويل ✓</span></div>
        ) : (
          <form onSubmit={convertToClient} className="grid gap-3 md:grid-cols-2">
            <div className="md:col-span-2"><h3 className="font-black">بيانات العميل</h3><p className="mt-1 text-sm text-slate-600">البيانات منقولة تلقائيًا من الإحالة. راجعها وعدّل فقط عند الحاجة.</p></div>
            ...
            <label className="md:col-span-2 flex items-center gap-2 rounded-xl border border-[#E5DED0] bg-white p-3 text-sm font-bold"><input type="checkbox" name="sendInvite" /> إرسال دعوة إلى العميل</label>
            <div className="md:col-span-2 flex flex-wrap items-center gap-3">
              <button disabled={draft.status !== "INTERESTED" || draft.adminDecision !== "ACCEPTED" || !referral.email || conversionBusy || conversionSucceeded} className="rounded-xl bg-[#B89A5A] px-6 py-3 font-black text-[#111827] transition hover:bg-[#C6AA69] disabled:cursor-not-allowed disabled:opacity-50">{conversionBusy ? "جارٍ التحويل..." : conversionSucceeded ? "تم التحويل ✓" : "تحويل إلى عميل"}</button>
              {conversionMessage && <div role={conversionSucceeded ? "status" : "alert"} className={`grid min-h-20 w-32 place-items-center rounded-2xl border p-3 text-center text-sm font-black shadow-sm ${conversionSucceeded ? "animate-pulse border-emerald-200 bg-emerald-50 text-emerald-800" : "border-rose-200 bg-rose-50 text-rose-800"}`}>{conversionMessage}</div>}
            </div>
            <p className="md:col-span-2 text-xs leading-6 text-slate-500">يصبح زر التحويل متاحًا عند ضبط حالة الإحالة على «مهتم» وقرار الإدارة على «مقبولة».</p>
          </form>
        )}
      </section>

      <div className="order-2 grid gap-5 p-5 xl:grid-cols-3">
        <section className="rounded-2xl border border-sky-100 bg-sky-50/40 p-4">
          ...
          <label className="mt-4 grid gap-2 text-sm font-bold">حالة الإحالة<select disabled={Boolean(referral.convertedClient)} value={draft.status} onChange={(event) => setDraft((current) => ({ ...current, status: event.target.value as ReferralStatus }))} className="rounded-xl border border-sky-200 bg-white p-3 font-normal disabled:opacity-60">{(Object.keys(referralLabels) as ReferralStatus[]).filter((status) => status !== "CONVERTED" || Boolean(referral.convertedClient)).map((status) => <option key={status} value={status}>{referralLabels[status]}</option>)}</select></label>
        ...
        <section className="rounded-2xl border border-amber-100 bg-amber-50/40 p-4">
          ...
          <label className="mt-4 grid gap-2 text-sm font-bold">القرار<select value={draft.adminDecision} onChange={(event) => changeDecision(event.target.value as ReferralDecision)} className="rounded-xl border border-amber-200 bg-white p-3 font-normal">{(Object.keys(decisionLabels) as ReferralDecision[]).filter((decision) => decision !== "CONVERTED_TO_CLIENT" || Boolean(referral.convertedClient)).map((decision) => <option key={decision} value={decision}>{decisionLabels[decision]}</option>)}</select></label>
          <label className="mt-3 grid gap-2 text-sm font-bold">ملاحظات الإدارة<textarea value={draft.adminNotes} onChange={(event) => setDraft((current) => ({ ...current, adminNotes: event.target.value }))} rows={4} placeholder="تفاصيل التواصل أو سبب القرار عند الحاجة" className="resize-y rounded-xl border border-amber-200 bg-white p-3 font-normal" /></label>
        </section>
        ...
        <section className="rounded-2xl border border-emerald-100 bg-emerald-50/40 p-4">
          ...
          {!commissionAllowed ? <p className="mt-4 rounded-xl bg-white p-4 text-sm leading-6 text-slate-600">اعتمد الإحالة أولًا أو حوّلها إلى عميل قبل إدخال العمولة.</p> : <div className="mt-4 grid gap-3">
            <label className="grid gap-2 text-sm font-bold">نوع العمولة<select value={draft.commissionType || ""} onChange={(event) => setDraft((current) => ({ ...current, commissionType: (event.target.value || null) as CommissionType | null, commissionAmount: null, commissionRate: null }))} className="rounded-xl border border-emerald-200 bg-white p-3 font-normal"><option value="">دون عمولة محددة</option><option value="FIXED">مبلغ ثابت</option><option value="PERCENTAGE">نسبة مئوية</option></select></label>
            ...
            <label className="grid gap-2 text-sm font-bold">حالة العمولة<select value={draft.commissionStatus} onChange={(event) => setDraft((current) => ({ ...current, commissionStatus: event.target.value as CommissionStatus }))} className="rounded-xl border border-emerald-200 bg-white p-3 font-normal">{(Object.keys(commissionLabels) as CommissionStatus[]).map((status) => <option key={status} value={status} disabled={(status === "DUE" || status === "PAID") && !referral.clientProject?.hasPaidInvoice}>{commissionLabels[status]}</option>)}</select></label>
      ...
      {referral.notes && <section className="order-3 mx-5 mb-5 rounded-2xl border border-indigo-100 bg-indigo-50/40 p-4"><SectionTitle icon={MessageSquareText} title="تفاصيل الطلب" subtitle="المعلومات التي قدّمها العميل أو ولّدها مسار المصدر." /><p className="mt-4 whitespace-pre-wrap break-words rounded-xl bg-white p-4 text-sm leading-7 text-slate-700">{referral.notes}</p></section>}

      <footer className="order-4 flex flex-wrap items-center justify-between gap-4 border-t border-[#EEE7DA] px-5 py-4">
        ...
        {!conversionSucceeded && (!referral.convertedClient || hasUnsavedChanges) && <button disabled={busy} onClick={() => void onSave(referral, draft)} className="rounded-xl bg-[#111827] px-6 py-3 font-black text-white transition hover:bg-[#1F2937] disabled:cursor-wait disabled:opacity-50">{busy ? "جارٍ الحفظ..." : referral.convertedClient ? "حفظ التعديلات الإدارية" : "حفظ التغييرات"}</button>}
      </footer>
    </article>
  );
}
...
function SummaryFact({ label, value }: { label: string; value: ReactNode }) {
  return <div className="min-w-0"><p className="text-xs font-bold text-slate-500">{label}</p><div className="mt-2 truncate font-black text-[#111827]">{value}</div></div>;
}
