        ...
        <section className="mt-6 overflow-hidden rounded-2xl border border-[#D8D2C4] bg-white shadow-sm">
          ...
          {formOpen && (
            <form onSubmit={createInvoice} className="grid gap-4 border-t border-[#D8D2C4] p-6 md:grid-cols-2">
              ...
              {selectedProject && <details open className="rounded-xl border border-[#D8D2C4] bg-[#F7F3EB] p-4 md:col-span-2"><summary className="cursor-pointer font-black">ملخص المشروع والاتفاق</summary><div className="mt-4 grid gap-3 sm:grid-cols-2"><ProjectFact label="المشروع" value={selectedProject.title} /><ProjectFact label="الحالة والتقدم" value={`${selectedProject.status} — ${selectedProject.progress}%`} /><ProjectFact label="الخطة المالية" value={selectedProject.financialPlan || "غير محددة"} /><ProjectFact label="مراحل المشروع" value={selectedProject.stages || "غير محددة"} /><ProjectFact label="تفاصيل الاتفاق" value={selectedProject.agreementDetails || "غير محددة"} /><ProjectFact label="موعد المشروع" value={selectedProject.dueAt ? formatDate(selectedProject.dueAt) : "غير محدد"} /></div></details>}
              ...
              <label className="grid gap-2 font-bold md:col-span-2">تاريخ الاستحقاق<input name="dueAt" type="date" className="field font-normal" /></label>
