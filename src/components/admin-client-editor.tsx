      ...
      <div className="grid min-h-screen lg:grid-cols-[280px_1fr]">
        ...
        <section className="p-4 sm:p-7 lg:p-10">
          ...
          {!loading && client && section === "overview" && <div className="mt-7">
            ...
            <section className="mt-6 rounded-2xl border border-[#D8D2C4] bg-white p-6 shadow-sm"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Bell className="h-5 w-5" /><h2 className="text-xl font-black">آخر الإشعارات</h2></div><button onClick={() => setNotificationsOpen(true)} className="text-sm font-bold text-[#9A7D43]">عرض من الجرس</button></div><div className="mt-4 grid gap-2">{client.clientNotifications.slice(0, 5).map((item) => <div key={item.id} className={`rounded-xl p-4 ${item.readAt ? "bg-slate-50 text-slate-600" : "bg-amber-50 text-[#111827]"}`}><div className="flex items-start justify-between gap-3"><strong>{item.title}</strong><time dateTime={item.createdAt} dir="ltr" className="shrink-0 text-xs text-slate-500">{formatDate(item.createdAt)}</time></div>{item.body && <p className="mt-1 text-sm text-slate-500">{item.body}</p>}</div>)}{!client.clientNotifications.length && <p className="text-slate-500">لا توجد إشعارات بعد.</p>}</div></section>
          </div>}
          ...
          {!loading && client && section === "messages" && <div className="mt-7 grid gap-5">
            <ListEmpty empty={!client.clientMessages.length} text="لا توجد رسائل بعد.">{[...client.clientMessages].sort((a, b) => b.createdAt.localeCompare(a.createdAt)).map((message) => <article key={message.id} className="rounded-2xl border border-[#D8D2C4] bg-white p-5 shadow-sm"><div className="flex flex-wrap justify-between gap-3"><strong>{message.subject || (message.fromAdmin ? "تحديث من الإدارة" : "رسالة العميل")}</strong><span className="text-xs text-slate-500">{formatDateTime(message.createdAt)}</span></div><span className="mt-2 inline-block rounded-full bg-[#F7F3EB] px-3 py-1 text-xs font-bold">{message.fromAdmin ? "الإدارة" : "العميل"}</span><p className="mt-3 whitespace-pre-wrap leading-7 text-slate-600">{message.body}</p></article>)}</ListEmpty>
