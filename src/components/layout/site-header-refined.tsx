      ...
      <div className="cw-container flex h-24 items-center justify-between">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Primary">
          {HEADER_ITEMS.map((item) => {
            ...
            return (
              <button key={item.id} type="button" onClick={() => go(item.id)} className={cn("focus-ring group relative rounded-md px-3 py-2 text-sm font-medium transition-colors", active ? "text-ink" : "text-muted-foreground hover:text-ink")} aria-current={active ? "page" : undefined}>
                {navLabel(item.id)}
                <span className={cn("absolute inset-x-3 -bottom-[1px] h-[2px] bg-accent transition-transform duration-300", dir === "rtl" ? "origin-right" : "origin-left", active ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100")} aria-hidden />
              </button>
            );
          })}
        </nav>
            ...
            <SheetContent side={dir === "rtl" ? "left" : "right"} className="flex w-full max-w-sm flex-col border-border bg-background p-0">
              ...
              <nav className="flex flex-col px-3 py-4" aria-label="Mobile">
                {HEADER_ITEMS.map((item) => {
                  ...
                  const active = view === item.id;
                  return (
                    <button key={item.id} type="button" onClick={() => go(item.id)} className={cn("focus-ring flex items-center justify-between rounded-md px-4 py-4 text-start text-base transition-colors", active ? "bg-muted text-ink" : "text-muted-foreground hover:bg-muted/60 hover:text-ink")} aria-current={active ? "page" : undefined}>
                      <span className="flex items-center gap-3"><Icon className="h-5 w-5 text-accent" /><span className="font-medium">{navLabel(item.id)}</span></span>
