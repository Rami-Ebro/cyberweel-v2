...
const SERVICES_AR = [
  ...
  },
  {
    ...
    text: "نقيّم المخاطر ونقاط الضعف، ونساعدك على حماية المواقع والأنظمة والحسابات والبيانات بإجراءات عملية تناسب حجم مشروعك",
...
const SERVICES_EN = [
  ...
  },
  {
    ...
    text: "We assess risks and weaknesses and help protect sites, systems, accounts, and business data with practical safeguards suited to your scale",
    ...
    <div>
      <PageHeader
        ...
        title={
          <>
            ...
            <span className="mt-5 block text-accent sm:mt-6">{h.titleLine2}</span>
          </>
        }
        intro={
          isArabic
            ? "نفهم المشكلة أولًا، ثم نحدّد وننفّذ ما يحتاجه مشروعك فعلًا: موقع، منصة، نظام، أتمتة، أمن سيبراني، أو تطوير لما لديك"
            : "We understand the problem first, then define and execute what your business truly needs: a site, platform, system, automation, cybersecurity, or improvement to what you already have"
        }
        ...
        <div className="mx-auto mt-16 grid max-w-6xl gap-6 md:grid-cols-2 lg:grid-cols-3">
          {services.map((service, index) => (
            ...
            >
              ...
              <p className="mt-6 border-t border-border/70 pt-5 text-sm font-semibold leading-relaxed text-ink/75">
                {service.result}
              </p>
            </motion.article>
          ))}
      ...
      <Section tone="background">
        <SectionHeading
          ...
          intro={
            isArabic
              ? "قد تحتاج إلى بناء حل جديد، أو تحسين ما لديك، أو اكتشاف أن المشكلة ليست تقنية من الأساس. مهمتنا ليست بيعك أكبر مشروع، بل مساعدتك على اتخاذ القرار الصحيح وتنفيذه بطريقة تخدم تقدّمك"
              : "You may need a new solution, an improvement to what you have, or the realization that the problem is not technical at all. Our role is not to sell the largest project, but to identify and execute the right decision for your progress"
          }
          className="mx-auto"
        />
        <div className="mx-auto mt-12 grid max-w-6xl gap-5 md:grid-cols-2 lg:grid-cols-4">
          {delivery.map(([title, text], index) => (
            <motion.article
              ...
              className="rounded-xl border border-camel/25 bg-background p-7 text-center shadow-sm"
            >
              <span className="inline-flex h-9 w-9 items-center justify-center rounded-md bg-camel/10 font-display text-sm font-semibold text-accent">
                {String(index + 1).padStart(2, "0")}
              </span>
