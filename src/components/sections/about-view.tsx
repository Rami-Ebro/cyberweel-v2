...
const stepsEn = [
  ["Clarity", "We begin by understanding your current position through listening and honest assessment"],
  ["Decision", "Together we choose the path that fits your business reality and goal"],
  ["Progress", "Measured movement toward your goal through steady, sustainable steps"],
];
...
export function AboutView() {
  ...
  return (
    <div>
      ...
      <Section tone="floral"><div className="grid gap-10 lg:grid-cols-2"><FadeIn delay={0}><div className="h-full rounded-2xl border border-camel/30 bg-camel/5 p-8 sm:p-10"><Check className="h-5 w-5 text-accent" /><h3 className="mt-5 font-display text-2xl font-medium text-ink">{a.weAreTitle}</h3><ul className="mt-5 space-y-3">{a.weAre.map((w) => <li key={w} className="flex gap-3 text-base text-muted-foreground"><Check className="mt-1 h-4 w-4 shrink-0 text-accent" />{w}</li>)}</ul></div></FadeIn><FadeIn delay={0.12}><div className="h-full rounded-2xl border border-border bg-muted/40 p-8 sm:p-10"><X className="h-5 w-5 text-muted-foreground" /><h3 className="mt-5 font-display text-2xl font-medium text-ink">{a.weAreNotTitle}</h3><ul className="mt-5 space-y-3">{a.weAreNot.map((w) => <li key={w} className="flex gap-3 text-base text-muted-foreground"><X className="mt-1 h-4 w-4 shrink-0" />{w}</li>)}</ul></div></FadeIn></div></Section>
