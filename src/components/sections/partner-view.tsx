...
export function PartnerView() {
  ...
  const data = isArabic ? DATA_AR : DATA_EN;
  const [active, setActive] = useState<PathId | null>(null);

  const formFields = active === "partner"
    ? [
        { kind: "text" as const, name: "name", label: isArabic ? "الاسم" : "Name", required: true },
        { kind: "text" as const, name: "email", label: isArabic ? "البريد الإلكتروني" : "Email", type: "email", required: true },
        { kind: "text" as const, name: "specialty", label: isArabic ? "التخصص والخبرة" : "Specialty and experience", required: true, full: true },
        { kind: "textarea" as const, name: "portfolio", label: isArabic ? "عرّفنا بأعمالك وتفرغك" : "Tell us about your work and availability", required: true, full: true, rows: 5 },
      ]
    : [
        { kind: "text" as const, name: "name", label: isArabic ? "الاسم" : "Name", required: true },
        { kind: "text" as const, name: "email", label: isArabic ? "البريد الإلكتروني" : "Email", type: "email", required: true },
        { kind: "text" as const, name: "market", label: isArabic ? "البلد أو السوق" : "Country or market", required: true, full: true },
        { kind: "textarea" as const, name: "network", label: isArabic ? "صف لنا شبكة علاقاتك وطريقة الوصول إلى الفرص" : "Describe your network and how you reach opportunities", required: true, full: true, rows: 5 },
      ];

  return (
    <div>
      <PageHeader
        eyebrow={isArabic ? "انضم إلينا" : "Join us"}
        title={<>{isArabic ? "انضم إلى عائلة " : "Join the "}<span className="font-display font-bold text-accent">CyberWeel</span></>}
        intro={isArabic ? "مساران واضحان للتعاون، اختر الطريق الذي يناسب خبرتك وطموحك" : "Two clear ways to collaborate. Choose the path that fits your expertise and ambition"}
      />
      ...
      <Section tone="muted" className="!pt-0">
        <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-2">
          {(Object.keys(data) as PathId[]).map((id, index) => {
            ...
            return (
              <motion.article key={id} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col rounded-2xl border border-border bg-white p-8 sm:p-10">
                ...
                <ul className="mt-8 space-y-3">
                  {path.items.map((item) => <li key={item} className="flex gap-3 text-sm leading-6 text-muted-foreground"><Check className="mt-1 h-4 w-4 shrink-0 text-accent" />{item}</li>)}
                </ul>
                <button type="button" onClick={() => setActive(id)} className="focus-ring mt-8 inline-flex w-fit rounded-lg bg-ink px-5 py-3 text-sm font-semibold text-floral hover:bg-accent">{path.cta}</button>
              </motion.article>
            );
          })}
        </div>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-ink/70 p-4" role="dialog" aria-modal="true">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-background p-6 shadow-2xl sm:p-8">
            <div className="mb-6 flex items-start justify-between gap-4">
              <div><p className="text-sm font-bold text-accent">{data[active].eyebrow}</p><h2 className="mt-2 font-display text-2xl font-semibold text-ink">{data[active].cta}</h2></div>
              <button type="button" onClick={() => setActive(null)} className="focus-ring rounded-md p-2 text-muted-foreground hover:bg-muted"><X className="h-5 w-5" /></button>
            </div>
            <MailtoForm
              to={BRAND.email}
              subject={`${active === "partner" ? (isArabic ? "طلب شريك تنفيذ" : "Delivery partner application") : (isArabic ? "طلب سفير CyberWeel" : "CyberWeel ambassador application")} — CyberWeel`}
              submitLabel={data[active].cta}
              successMessage={isArabic ? "وصلت رسالتك بنجاح. شارك هذه الفرصة مع شخص قد تناسبه" : "Your message was sent successfully. Share this opportunity with someone it may suit"}
              allowAttachments={active === "partner"}
              fields={formFields}
            />
          </div>
        </div>
